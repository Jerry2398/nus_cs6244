...
#PBS -j oe
#PBS -o job_log_openvla_oft_libero_ft_eval_cs6244_prelim_one_gpu.out
...
RUN_ID="openvla-7b+cs6244_prelim+b8+lr-0.0005+lora-r32+dropout-0.0--image_aug--libero_ft_cs6244_prelim_1_gpu"
...
torchrun --standalone --nnodes 1 --nproc-per-node "${NPROC_PER_NODE}" vla-scripts/finetune.py \
  ...
  --image_aug True \
  --lora_rank 32 \
  --run_id_note "libero_ft_cs6244_prelim_1_gpu" \
...
# norm_stats in checkpoint use the training dataset name (cs6244_prelim), not libero_spatial
