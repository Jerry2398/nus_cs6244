...
#PBS -l select=1:ncpus=32:mpiprocs=1:ompthreads=32:mem=128gb:ngpus=4
#PBS -j oe
#PBS -o job_log_openvla_oft_libero_ft_eval_formal.out
...
torchrun --standalone --nnodes 1 --nproc-per-node "${NPROC_PER_NODE}" vla-scripts/finetune.py \
  ...
  --image_aug True \
  --lora_rank 32 \
  --run_id_note "libero_ft_and_eval_formal" \
